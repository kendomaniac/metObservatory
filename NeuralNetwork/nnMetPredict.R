...
#'}
#' @export
nnMetPredict <- function(group=c("sudo","docker"), scratch.folder, file,separator,patiencePercentage=5,projectName,bN="NULL",eV,BW){
...
system(paste("cp ",BW," ",scrat_tmp.folder,"/",projectName,"/BW/",sep=""))
  ...
  #executing the docker job
    params <- paste("--cidfile ",data.folder,"/dockerID -v ",scrat_tmp.folder,":/scratch -v ", data.folder, ":/data -d repbioinfo/neuralnetworkmet python3 /home/evaluate.py ",matrixNameC,".",format," ",separator," ",patiencePercentage," ",projectName," ",basename(eV)," ",basename(bN),sep="")
